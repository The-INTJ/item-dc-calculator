...
import { CreateContestantBodySchema } from '@/contest/lib/schemas';
...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  const result = await provider.contestants.listByContest(contest.id);
  if (!result.success) {
    return jsonError(result.error ?? 'Contestants not found', 404);
  }
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const body = await parseBody(request, CreateContestantBodySchema);
  if (!body.ok) {
    return body.response;
  }

  const result = await provider.contestants.create(contest.id, body.data);
