 ...
 */
export function openMatchupEntries(matchupSnap: DocumentSnapshot): Entry[] {
...
export function writeVoteAndTally(
  ...
  const delta = isNewVote
    ? newTotal
    : newTotal -
      computeVoteTotal(((voteSnap.data() as Record<string, unknown>)?.breakdown ?? {}) as ScoreBreakdown);

  const voteData: Record<string, unknown> = { ...vote, updatedAt: FieldValue.serverTimestamp() };
