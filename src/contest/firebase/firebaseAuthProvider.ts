...
const USERS_COLLECTION = 'contest_users';
const VOTES_COLLECTION = 'contest_votes';
  ...
  return {
    ...
    async fetchUserData(
      ...
      try {
        ...
        const votes: UserVote[] = votesSnapshot.docs.map((voteDoc) => {
          const data = voteDoc.data();
          return {
            contestId: data.contestId,
            entryId: data.entryId ?? data.drinkId, // Support legacy drinkId
            score: data.score,
    ...
    async saveVote(uid: string, vote: UserVote): Promise<AuthResult> {
      ...
      try {
        // Check if vote already exists for this user/contest/entry
        const votesQuery = query(
          collection(db, VOTES_COLLECTION),
          where('userId', '==', uid),
