...
/**
 * Auth context and provider for Contest system.
 *
...
export function ContestAuthProvider({ children }: AuthProviderProps) {
...
// Legacy alias
export const MixologyAuthProvider = ContestAuthProvider;
...
export function useAuth(): AuthContextValue {
  ...
  if (!context) {
    throw new Error('useAuth must be used within ContestAuthProvider');
  }
  return context;
}
...
function mergeVotes(local: UserVote[], remote: UserVote[]): UserVote[] {
  ...
  for (const vote of remote) {
    const key = `${vote.contestId}:${vote.entryId}`;
    merged.set(key, vote);
  }
