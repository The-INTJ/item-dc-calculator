...
const STORAGE_KEY = 'contest_session';
const SESSION_VERSION = 2; // Bumped for new schema
...
export function addVoteToSession(vote: UserVote): LocalSession | null {
  ...
  // Check if vote already exists for this entry (update it)
  const existingIdx = current.votes.findIndex(
    (v) => v.contestId === vote.contestId && v.entryId === vote.entryId
  );
