...
## Goals
- **Shared tokens, separate themes.** Keep a single source of truth for spacing, typography, and semantic color tokens (Sass-driven) while allowing the Mixology and Legacy (D&D calculator) apps to look distinct.
...
- Theme separation is now driven by body classes (`theme-mixology` and `theme-legacy`) that map to shared Sass token values.
- Container queries are available for the primary layouts and a few legacy components, but not fully migrated.

## Architecture Plan
### 1) Tokens
**Source of truth:** `src/variables.scss` for Sass variables, plus theme-scoped rules in `app/globals.scss` for runtime theming.

**Next steps:**
- Define semantic Sass variables for typography scale, spacing steps, radii, and elevations.
- Map legacy colors to the semantic token layer (Sass-driven values scoped in `app/globals.scss`).
- Replace direct hex usage in mixology-specific styles with semantic Sass variables.
