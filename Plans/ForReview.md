...
- **Auth "session hydrate" logic is repeated across login flows.** `login`, `loginWithGoogle`, and `loginAnonymously` all build near-identical `LocalSession` objects, merge guest votes, and persist to storage. This looks like a single helper (e.g., `buildSyncedSession` + `mergeGuestVotes`) that is currently copy/pasted in three places, increasing the chance of drift (e.g., profile defaults). See `AuthContext.tsx`.
